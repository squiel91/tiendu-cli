...
const uploadPreviewZipWithRetries = (apiBaseUrl, apiKey, storeId, previewKey, zipBuffer, onRetry) =>
  retryAsync(
...
export const dev = async () => {
  ...
  spinner.message("Syncing current files...");
  const zipBuffer = await createZipFromDirectory(rootDir);
  const uploadResult = await uploadPreviewZipWithRetries(
    apiBaseUrl,
    apiKey,
    storeId,
    previewKey,
    zipBuffer,
    async (result, nextAttempt) => {
      spinner.message(
        `Initial sync failed. Retrying ${nextAttempt}/${RETRY_ATTEMPTS}... ${result.error}`,
      );
    },
  );
